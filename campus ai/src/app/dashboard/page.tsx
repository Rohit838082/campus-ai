...
export default async function DashboardPage() {
  let me = mockProfile as typeof mockProfile | null;
  let recentDoubts = mockDoubts.slice(0, 3) as typeof mockDoubts;
  let noteCount = { count: mockNotes.length };
  let pyqCount = { count: mockPyqs.length };
  let doubtCount = { count: mockDoubts.length };

  try {
    if (db) {
      const [dbMe] = await db.select().from(profile).limit(1);
      const dbRecentDoubts = await db.select().from(doubts).orderBy(desc(doubts.createdAt)).limit(3);
      const [dbNoteCount] = await db.select({ count: sql<number>`count(*)` }).from(notes);
      const [dbPyqCount] = await db.select({ count: sql<number>`count(*)` }).from(pyqs);
      const [dbDoubtCount] = await db.select({ count: sql<number>`count(*)` }).from(doubts);
      if (dbMe) me = dbMe;
      if (dbRecentDoubts) recentDoubts = dbRecentDoubts as typeof mockDoubts;
      if (dbNoteCount) noteCount = dbNoteCount;
      if (dbPyqCount) pyqCount = dbPyqCount;
      if (dbDoubtCount) doubtCount = dbDoubtCount;
    }
  } catch {
    /* use mock fallback data */
  }

  if (!me) me = mockProfile;
